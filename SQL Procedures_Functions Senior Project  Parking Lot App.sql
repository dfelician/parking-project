exec updateLot01Proc -- Updates lot01 sets available to 'Y', username = NULL, ramID NULL. Runs at midnight with the query: 'IF(CONVERT(VARCHAR(8),GETDATE(),108) LIKE '00:00%')'. Neccessary to reset lots every night. Program automatically will run at midnight using Windows Scheduler. 
...
exec updateLot02Proc --Updates lot02 sets available to 'Y', username = NULL, ramID NULL.Runs at midnight with the query: 'IF(CONVERT(VARCHAR(8),GETDATE(),108) LIKE '00:00%')'. Neccessary to reset lots every night. Program automatically will run at midnight using Windows Scheduler. 
...
exec updateLot03Proc --Updates lot03 sets available to 'Y', username = NULL, ramID NULL.Runs at midnight with the query: 'IF(CONVERT(VARCHAR(8),GETDATE(),108) LIKE '00:00%')'. Neccessary to reset lots every night. Program automatically will run at midnight using Windows Scheduler. 
...
exec AdminReserveUpdate --Updates course to set start_time equal to the current time, in order to get around the CK_Lot01 check constraint. Mainly used as a nested proc.
--Syntax of Proc
/*
...
exec MorningAdminProcedure --Procedure used for reserving multiple spots if the date in dbo.PendingRequests is equal to the current date (GETDATE()). Will reserve the number of spots ranging from 1 to x depending on the number of spots reserved in dbo.PendingRequests
...
exec dbo.Lot01CheckCsontraintFunction @RamID = 'R01632755'

Function mainly used for the Lot01 check constraint. If this function returns true, allow a user to reserve a spot in lot01. This function checks to see if a student has a class within 15 minutes before the class starts, for that specific @RamID
...
END
*/
@ToyinIku#3450  Since i never put any database/SQL things on github. @here if you want to reference them later, or required for resume
